"""
Denetim Turu 3 — Kalan Düzeltmeler
K2: NAHCIivan yazımı + TAP- prefix
...
for rk in top_regions:
    ...
    # Fix telemetry entity_ids and keys
    new_telemetry = {}
    for tid, telem in region.get('telemetry', {}).items():
        eid = telem.get('entity_id', '')
        new_eid = eid.replace('NAHCIivan', 'NAHCIVAN')
        if new_eid != eid:
            fixes.append(f"K3 Telemetry entity_id: {eid} → {new_eid}")
            telem['entity_id'] = new_eid
        
        expected_key = new_eid.replace('-', '_').lower()
        if tid != expected_key:
            fixes.append(f"K3 Telemetry key: {tid} → {expected_key}")
        
        new_telemetry[expected_key] = telem
    
    region['telemetry'] = new_telemetry

# ════════════════════════════════════════════════════════════════════════════
# K2 — TAP- prefix → CS-TAP- (regex'e uyumlu hale getir)
# ════════════════════════════════════════════════════════════════════════════
...
for rk in top_regions:
    ...
    for eq in region.get('equipment', []):
        ...
        if old.startswith('TAP-'):
            new = old.replace('TAP-', 'CS-TAP-', 1)
            fixes.append(f"K2 Equipment: {old} → {new}")
            eq['unit_id'] = new
    
    # Fix telemetry entity_ids for TAP equipment
    new_telemetry = {}
    for tid, telem in region.get('telemetry', {}).items():
        eid = telem.get('entity_id', '')
        if eid.startswith('TAP-'):
            new_eid = eid.replace('TAP-', 'CS-TAP-', 1)
            fixes.append(f"K3 Telemetry entity_id: {eid} → {new_eid}")
            telem['entity_id'] = new_eid
...
# ════════════════════════════════════════════════════════════════════════════
# K1 — Meta'yı yeniden hesapla (son kez)
# ════════════════════════════════════════════════════════════════════════════
